...
| `survey_photos.yaml` | Generated manifest consumed by the web app |
...
- **`GET /api/photos`** — JSON list loaded from `survey_photos.yaml` (cached in memory after the first request).
- **`GET /api/track`** — JSON `{"coordinates": [[lat, lon], ...]}` from all GPX `trkpt` points in **`track/MHCG-HITW-SURVEY.gpx`** (cached after the first request).
- **`GET /media/{filename}`** — serves files from **`photos/`** by basename only (path traversal is rejected).
- **`GET /`** — HTML UI: filterable list, metadata, photo preview, and two **Leaflet** maps (**OpenTopoMap**): one fitted to the full hike track, one zoomed to the selected photo’s coordinates (map tiles need network access in the browser).
...
Compose maps host **8765** to container **8000** and mount **`./survey_photos.yaml`**, **`./photos`**, and **`./track`** read-only into the container, so the image stays small and you can refresh data on disk.
...
docker run --rm -p 8765:8000 \
  -v "$(pwd)/survey_photos.yaml:/work/survey_photos.yaml:ro" \
  -v "$(pwd)/photos:/work/photos:ro" \
  -v "$(pwd)/track:/work/track:ro" \
  survey-viewer
```

The image also copies **`track/`** at build time, so the last volume is optional unless you want to override the baked-in GPX.
